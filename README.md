...
- 框架介绍视频：
https://b23.tv/D5pVysd
- 合规声明：严禁使用本项目开发违规、违法用途的机器人，违者后果自负
- 加密说明：核心代码加密处理，仅为防止二次修改后商用售卖，保障原创权益，感谢理解

## 环境搭建与对接流程
1. 环境要求：必须使用PHP 7.4版本，推荐使用宝塔面板管理，需在宝塔PHP拓展管理中安装sg16拓展；切记php要7.4且sg16安装完成后重启php

2. 对接步骤：登录QQ官方机器人平台：
https://q.qq.com/
进入「开发设置-回调配置」，订阅方式选「WebHook」，填写框架对应服务回调地址（仅支持80、443、8080、8443端口），勾选所需监听事件（建议全选），提交后等待官方验证通过即可使用。

3.回调地址配置模板：
https://您的域名或已解析的IP/webhook.php?bot=appid
请替换为您的域名或已解析的IP及机器人AppID，配置前确保域名可正常访问且已完成备案【需开启SSL】


## 修复数据库问题
1.2025年12月11日22:09修复数据库问题导致bot添加不了
